...
// [[Rcpp::export]]
cube gof_iter_fct(const vec& H, const mat& d, const mat& q, const mat& prev, int nquar, int nage){
  // Timer timer;
  ...
  vector<vector<arma::mat> > PA(nquar, vector<arma::mat>(nage));
  ...
  for(int t0=0; t0<nquar; ++t0){
    for(int a0=0; a0<nage; ++a0){
      ...
      if(t0==0){
          ...
          lat_p = lat_p + (PA[0][a0].t() * tmp);
          //Rcpp::Rcout << "lat_p:" << std::endl << lat_p << std::endl;
          
          // Diagnosed initial prevalence
          diag_p(0) = tmp(0) * d(0,0) ;
          diag_p(1) = tmp(1) * d(1,0) ;
          diag_p(2) = tmp(2) * d(2,0) ;
          diag_p(3) = tmp(3) * d(3,0) ;
          diag_p(4) = tmp(3) * (1-d(3,0)) * q(3,a0) ;
      ...
      for(int t=(t0+1); t<nquar; ++t){
        ...
        lat_arr[t][a0][t0] = PA[t][a0].t()* lat_arr[t-1][a0][t0];

        vec tmpvec(5);

        tmpvec[0] = lat_arr[t-1][a0][t0][0] * d(0,t) ;
        tmpvec[1] = lat_arr[t-1][a0][t0][1] * d(1,t) ;
        tmpvec[2] = lat_arr[t-1][a0][t0][2] * d(2,t) ;
        tmpvec[3] = lat_arr[t-1][a0][t0][3] * d(3,t) ;
        tmpvec[4] = lat_arr[t-1][a0][t0][3] * (1-d(3,t)) * q(3,a0) ;
...
// [[Rcpp::export]]
cube prev_iter_fct(const vec& H, const mat& d, const mat& q, const mat& prev, int nquar, int nage){
  // Timer timer;
