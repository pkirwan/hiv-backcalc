...
// CAN STILL ADD THE ANALYTIC CASE FOR THIS VERSION
...
// [[Rcpp::export]]
vec probs_move_fct_t_to_d(int t0, int a0, int st, int t, int nt, int nage, const mat& d, const mat& q){
  ...
  for(int i=0; i<time_to_end; ++i){
    ...
    probs0(i) = d(st-1,t-1) + (1-d(st-1,t-1))*q(st-1,a0-1); // all the -1 are for consistency with R
  }
...
// [[Rcpp::export]]
vec probs_move_fct_snap(int t0, int a0, int st, int t, int nt, int nage, int t_max, const mat& d, const mat& q){
  if(t < t0) {throw std::range_error("t cannot be smaller than t0");}
  if(st==1 & t!=t0) {throw std::range_error("In the first state t must be equal to t0");}
  vec probs(t_max);
  vec probs0(t_max);
  ...
  for(int i=0; i<t_max; ++i){
    ...
    // Defining probabilities of moving (either dx or prog)
    // In state k, and at time t (t0 + i)
    probs0(i) = d(st-1,t0-1) + (1-d(st-1,t0-1))*q(st-1,a0-1); // all the -1 are for consistency with R
  }
  const vec cp = cumprod(1-probs0);
  vec one_vec=vec(1);
  one_vec(0)=1;
...
// [[Rcpp::export]]
double probs_diag_fct_t_to_d(int t0, int a0, int st, int t, int nt, int nage, const mat& d, const mat& q){
  double dd = d(st-1,t-1);
...
// [[Rcpp::export]]
double probs_diag_fct_snap(int t0, int a0, int st, int t, int nt, int nage, const mat& d, const mat& q){
  double dd = d(st-1,t0-1);
  double d_star = dd + (1-dd)*q(st-1,a0-1); // all the -1 are for consistency with R
  double out = dd / d_star;
  return(out);
}
...
// [[Rcpp::export]]
vec one_sim_t_to_d(int t0, int a0, int nt, int nage, const mat& d, const mat& q){
...
// [[Rcpp::export]]
vec one_sim_snap(int t0, int a0, int nt, int nage, int t_max, const mat& d, const mat& q){
  // Initializing quantities
  int dx_time = NA_INTEGER;
  int st_dx = 0; // NA would be better but creates issues later ... st_dx=0 means no diag
  int k = 1;
