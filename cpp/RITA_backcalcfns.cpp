...
// [[Rcpp::export]]
cube trans_mat(const arma::vec& d1, const arma::vec& d2, const arma::vec& d3, const arma::vec& d4, const arma::vec& d5, const arma::vec& q){
  ...
  for(int t=0; t<t_max;++t){
    ...
    vec d(5);
    d(0) = d1(t); d(1) = d2(t); d(2) = d3(t); d(3) = d4(t); d(4) = d5(t);

    trans_mats(0,0,t) = 0;
    trans_mats(0,1,t) = (1-q(0))*(1-d(0));
    trans_mats(0,2,t) = q(0)*(1-d(0));
    trans_mats(0,8,t) = d(0);

    trans_mats(1,1,t) = 0;
    trans_mats(1,3,t) = (1-q(0))*(1-d(0));
    trans_mats(1,4,t) = q(0)*(1-d(0));
    trans_mats(1,8,t) = d(0);

    trans_mats(2,2,t) = 0;
    trans_mats(2,4,t) = (1-q(1))*(1-d(0));
    trans_mats(2,5,t) = q(1)*(1-d(0));
    trans_mats(2,8,t) = d(0);

    for(int j=3; j<ncd4_st;++j){
      trans_mats(j,j,t) = (1-q(j-3))*(1-d(j-2));
      trans_mats(j,j+1,t) = q(j-3)*(1-d(j-2));
      trans_mats(j,j+6,t) = d(j-2);
    }
...
mat prev_t(int t, const arma::vec& CD4_prev, const arma::vec& h, const cube& trans_mats){
  ...
  // Rcpp::Rcout << prev << std::endl;

  return prev;
}

// [[Rcpp::export]]
List forward_calculate(const arma::vec& CD4_prev, const arma::vec& h, const arma::vec& d1, const arma::vec& d2, const arma::vec& d3, const arma::vec& d4, const arma::vec& d5, const arma::vec& q){
  int t_max = h.n_elem;
  int ncd4_st = 7;
  int nst = 13;

  cube out(t_max, nst, t_max, fill::zeros);
  cube tmats = trans_mat(d1, d2, d3, d4, d5, q);
  // Rcpp::Rcout << tmats << std::endl;
