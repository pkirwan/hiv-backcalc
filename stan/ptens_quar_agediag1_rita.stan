...
// AGE DEPENDENT DIAGNOSES 
// Age and state specific linear trend
// in the logistic rw for dx probs (for each state)

// HERE: Have age and state specific coefficient for a linear trend
...
functions{
  ...
  matrix Q_fct(matrix d, matrix q, int t, int a){
    matrix[7,7] Q = rep_matrix(0, 7, 7); // generate a zero matrix
    
    Q[1,2] = (1 - d[1,t]) * (1 - q[1,a]) ;
    Q[1,3] = (1 - d[1,t]) * q[1,a] ;
    
    Q[2,4] = (1 - d[1,t]) * (1 - q[1,a]) ;
    Q[2,5] = (1 - d[1,t]) * q[1,a] ;

    Q[3,5] = (1 - d[1,t]) * (1 - q[2,a]) ;
    Q[3,6] = (1 - d[1,t]) * q[2,a] ;
  ...
  matrix[,] diag_arr_fct(matrix[,] diag_arr, matrix e_m, int nquar, int nage){
    matrix[nquar,6] diag_arr1[nquar,nage];
    row_vector[6] out[nquar,nage];
    row_vector[6] x;
    int a; 
...
data{
  ...
  int<lower=0> CD4[nquar,nage,5] ; // CD4 data by number of years and ages and CD4 states(3)
  matrix<lower=0>[7,nage] init_prev ; // Initial prevalence by age at inf and CD4 states(3) 
...
transformed data{
  vector[6] empty_vec = rep_vector(0,6);
  row_vector[6] empty_vec1 = rep_row_vector(0,6);
  vector[7] empty_vec2 = rep_vector(0,7);
  matrix[nquar,6] empty_mat = rep_matrix(0,nquar,6);
...
parameters{
  ...
  vector<lower=0>[5] vardelta;
  vector[nquar] delta_raw1; 
  vector[nquar] delta_raw2; 
  vector[nquar] delta_raw3; 
  vector[nquar] delta_raw4; 
  vector[nquar] delta_raw5;
  vector[5*nagroup] alpha; // age group specific (ag) and state (k) specific linear time trend for dx probs --- 
}
...
transformed parameters{
  ...
  matrix<lower=0, upper=1>[7,7] PA[nquar, nage, nagroup];
  vector<lower=0>[7] lat_arr[nquar]; // expected number of diag at time t age a state k
  vector<lower=0>[7] lat_p;
  row_vector<lower=0>[6] diag_p;
  matrix[nquar, 6] diag_arr[nquar,nage]; // expected number of diag at time t age a state k
  matrix<lower=0>[nquar, 6] diag_arr1[nquar,nage]; // expected number of diag at time t age a state k
  vector<lower=0>[7] temp; // expected number of diag at time t age a state k
  row_vector<lower=0>[6] exp_diag[nquar,nage]; // expected number of diag at time t age a state k
  ...
  simplex[5] p_tak[nquar,nage]; // proportion of HIV diagnoses in state k (time t and age a)
  matrix[5,nquar] d[4];// matrix of diagnoses for different ages and age-groups
  ...
  // Initializing delta
  delta1[1] = -3.2 + 0.2*delta_raw1[1];
  delta2[1] = -3.2 + 0.2*delta_raw2[1]; // worth reconsidering? Originally for diag in 85
  delta3[1] = -3.2 + 0.2*delta_raw3[1]; // worth reconsidering? Originally for diag in 85
  delta4[1] = -3.0 + 0.2*delta_raw4[1]; // worth reconsidering? Originally for diag in 85
  delta5[1] = -2.5 + 0.3*delta_raw5[1]; // worth reconsidering? Originally for diag in 85
  ...
  for(t in 2:nquar){
    ...
    delta5[t] = delta5[t-1] + vardelta[5]*delta_raw5[t];
  }
  ...
  // Age-specific additive term, interpreted as a linear time trend (for a given state)
  // which has an age and time specific gradient
  for(ag in 1:nagroup){
    for(i in 1:nquar){
      d[ag,1,i] = inv_logit(delta1[i]+ alpha[(ag-1)*nagroup+1]);
      d[ag,2,i] = inv_logit(delta2[i]+ alpha[(ag-1)*nagroup+2]);
      d[ag,3,i] = inv_logit(delta3[i]+ alpha[(ag-1)*nagroup+3]);
      d[ag,4,i] = inv_logit(delta4[i]+ alpha[(ag-1)*nagroup+4]);
      d[ag,5,i] = inv_logit(delta5[i]+ alpha[(ag-1)*nagroup+5]);
    }
  }
  ...
  for(t0 in 1:nquar){
    for(a0 in 1:nage){
      ...
      if(t0 == 1){
        ...
        diag_p[1] = (init_prev[1,a0] * d[ag0,1,1]) + (init_prev[2,a0] * d[ag0,1,1]) + (init_prev[3,a0] * d[ag0,1,1]) ;
        diag_p[2] = init_prev[4,a0] * d[ag0,2,1] ;
        diag_p[3] = init_prev[5,a0] * d[ag0,3,1] ;
        diag_p[4] = init_prev[6,a0] * d[ag0,4,1] ;
        diag_p[5] = init_prev[7,a0] * d[ag0,5,1] ;
        diag_p[6] = init_prev[7,a0] * (1-d[ag0,5,1]) * q[4,a0] ;
      }
      lat_arr[t0,1] = H[(a0-1)*nquar+t0] + lat_p[1];
      lat_arr[t0,2:7] = empty_vec + lat_p[2:7] ;
      ...
      for(t in (t0+1):nquar){ // might worry if t = nquar, but in STAN loops in (nquar+1):nquar are not evaluated (unlike R, like JAGS) --- saving an if loop
        ...
        diag_arr[t,a0,t0,1] = (temp[1] * d[ag,1,t]) + (temp[2] * d[ag,1,t]) + (temp[3] * d[ag,1,t]) ;
        diag_arr[t,a0,t0,2] = temp[4] * d[ag,2,t] ;
        diag_arr[t,a0,t0,3] = temp[5] * d[ag,3,t] ;
        diag_arr[t,a0,t0,4] = temp[6] * d[ag,4,t] ;
        diag_arr[t,a0,t0,5] = temp[7] * d[ag,5,t] ;
        diag_arr[t,a0,t0,6] = temp[7] * (1-d[ag,5,t]) * q[4,a0] ;
      }
    }
  }
  ...
  for(t in 1:nquar){
    for(a in 1:nage){
      ...
      exp_HIV[t,a] = exp_diag[t,a,1] + exp_diag[t,a,2] + exp_diag[t,a,3] + exp_diag[t,a,4] + exp_diag[t,a,5]; 
      // Expected AIDS diagnoses
      exp_AIDS[t,a] = exp_diag[t,a,6]; 
    }
  }
  ...
  for(t in 1:nquar){
    for(a in 2:nage){ 
      ...
      p_tak[t,a,5] = exp_diag[t,a,5] / exp_HIV[t,a];
    }
  }
