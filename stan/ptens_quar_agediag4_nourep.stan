...
// AGE DEPENDENT DIAGNOSES 
// Age specific intercept that affects all the diag rw
// constant through states 1-4
...
parameters{
  ...
  vector[4] alpha; // age (ac) specific intercept for dx probs --- 
}
...
transformed parameters{
  ...
  matrix[nquar,nagroup] delta1; 
  matrix[nquar,nagroup] delta2; 
  matrix[nquar,nagroup] delta3; 
  matrix[nquar,nagroup] delta4; 
  ...
  for(ag in 1:nagroup){
    delta1[1,ag] = -3.2 + 0.2*delta_raw1[1] + alpha[ag]; // worth reconsidering? Originally for diag in 85
    delta2[1,ag] = -3.2 + 0.2*delta_raw2[1] + alpha[ag]; // worth reconsidering? Originally for diag in 85
    delta3[1,ag] = -3.0 + 0.2*delta_raw3[1] + alpha[ag]; // worth reconsidering? Originally for diag in 85
    delta4[1,ag] = -2.5 + 0.3*delta_raw4[1] + alpha[ag]; // worth reconsidering? Originally for diag in 85
  }
