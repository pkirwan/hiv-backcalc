...
data{  
int nquar; // number of years   
int nage; // number of ages   
int ninfpars; // number of infection parameters   
int nmissing; // number of quarters for which we want the posterior predictive distribution.  
int sigma_inform; // should we use an informative prior on the smoothing parameter of the incidence spline  
int HIV[nquar,nage] ; // data with new HIV diag by year and age  
int AIDS[nquar,nage] ; // data with new AIDS diag by year and age  
int CD4[nquar,nage,4] ; // CD4 data by number of years and ages and CD4 states(3)  
...
vector[2] sigma; // smoothing parameters  
vector[4] vardelta;  
...
vector[(nquar+nmissing)*nage] H; //vector of expected log responses  
matrix[4,4] PA[nquar+nmissing, nage, nagroup];  
vector[4] lat_arr[nquar+nmissing]; // expected number of diag at time t age a state k  
vector[4] lat_p;  
row_vector[5] diag_p;  
matrix[nquar+nmissing, 5] diag_arr[nquar+nmissing,nage]; // expected number of diag at time t age a state k  
matrix[nquar+nmissing, 5] diag_arr1[nquar+nmissing,nage]; // expected number of diag at time t age a state k  
vector[4] temp; // expected number of diag at time t age a state k  
row_vector[5] exp_diag[nquar+nmissing,nage]; // expected number of diag at time t age a state k  
...
vector[2] lambda; // smoothing parameters  
...
// New infs enter model in state 1  
      ...
      for(t in (t0+1):(nquar+nmissing)){ // might worry if t = nquar+nmissing, but in STAN loops in nmissing + (nquar+1):nquar are not evaluated (unlike R, like JAGS) --- saving an if loop
...
int HIV_sim[nmissing,nage]; // simulated missing HIV diagnosis data. Should this be declared as a matrix somehow?  
int AIDS_sim[nmissing,nage]; // simulated missing AIDS diagnosis data.  
for(t in 1:nquar){  
